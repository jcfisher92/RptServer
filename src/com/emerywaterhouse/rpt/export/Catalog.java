...
public class Catalog extends Report
{
   ...
   private String m_EscapeFormat;    // 'html' or 'xml'.  Determines which characters will be escaped in the final document.
   private boolean m_AllHazmat;	    // If true, all hazmat entries will be reported.  Otherwise only those with excepted=0 will be reported.
   private boolean m_IncludeAsst;    // If true, items with a ship unit of AST will be included.  Default is true;
   private String m_DescrSrc;		    // "item" = use item.description or "web" = use bmi_item.description
   ...
   private boolean buildXml(FileOutputStream outFile) throws Exception
   {
      ...
      try {
         ...
         while ( rsItemData.next() && m_Status == RptServer.RUNNING ) {
            ...
            if ( m_DescrSrc.equalsIgnoreCase("web") )
            	desc = rsItemData.getString("web_descr");
            else
            	desc = rsItemData.getString("description");
   ...
   private String getAttributes(int itemEaId) throws SQLException
   {
      ...
      try {
         ...
         log.error("[Catalog]#getAttributes]", ex);
      }
   ...
   private String getBullets(int itemEaId) throws SQLException
   {
      ...
      try {
         ...
         log.error("[Catalog]#getBullets", ex);
      }
   ...
   private String getCategories(int itemEaId) throws SQLException
   {
      ...
      try {
         ...
         log.error("[Catalog#getCategories] exception:", ex);
      }
   ...
   private String getHazardData(String itemId) throws SQLException
   {
      ...
      try {
         ...
         log.error("[Catalog#getHazardData] exception:", ex);
      }
   ...
   private String getLocationData(int itemEaId) throws SQLException
   {
      ...
      try {
         ...
         log.error("[Catalog#getLocationData] exception:", ex);
      }
   ...
   /**
    * Returns a normalize string.  The normalization can be either to xml
    * or html standard, depending on the value of m_EscapeFormat.  xml standard
    * only normalizes characters '<', '>', '&', and '"'.  Html standard also
    * normalizes special characters like trademark, copyright, etc.
    *
   ...
   private boolean prepareStatements()
   {
      ...
      if ( m_EdbConn != null ) {
         try {
            ...
            sql.append("where \r\n");
            sql.append("   item_entity_attr.item_type_id in (1, 9) ");
            ...
            sql.append("   ejd.retail_price_procs.getretailprice(?, item_entity_attr.item_id) as retail, \r\n");
            ...
            sql.append(m_AllHazmat ? " " : " and excepted = 'N' ");
   ...
   public void setParams(ArrayList<Param> params)
   {
      ...
      for ( int i = 0; i < pcount; i++ ) {
         ...
         if ( param.name.equals("allhazmat") )
         	m_AllHazmat = param.value.equalsIgnoreCase("true") ? true : false;
