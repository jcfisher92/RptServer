...
public class Rocksolid extends Report
{
   ...
   private boolean prepareStatements()
   {
      ...
      if ( m_EdbConn != null ) {
         try {
            ...
            sql.append("left join ejd_item_whs_upc on ejd_item_whs_upc.ejd_item_id = item_entity_attr.ejd_item_id and ejd_item_whs_upc.primary_upc = 1"
                  + " and ejd_item_whs_upc.warehouse_id = ejd_item_warehouse.warehouse_id ");            
            ...
            sql.append("union ");
            sql.append("select ");
            sql.append("   item_entity_attr.item_id, nvl(web_item_ea.web_descr, item_entity_attr.description) as description, ejd_item_whs_upc.upc_code, ");
            sql.append("   vic.vendor_item_num, vendor.vendor_id, vendor.name, item_entity_attr.ejd_item_id, ejd_item_warehouse.warehouse_id, ");
            sql.append("   'http://www.emeryonline.com/shared/images/catalog/' || web_item_ea.main_image || '_sm.gif' img_url_sm, ");
            sql.append("   'http://www.emeryonline.com/shared/images/catalog/' || web_item_ea.main_image || '.gif' img_url_md, ");
            sql.append("   'http://www.emeryonline.com/shared/images/catalog/' || web_item_ea.main_image || '_lg.gif' img_url_ld, ");
            sql.append("   decode(bc.description, 'ALLOW BROKEN CASES', 'Y', 'N') as broken_case, ");
            sql.append("   decode(bc.description, 'ALLOW BROKEN CASES', 1, ejd_item_warehouse.stock_pack) as packof, ");
            sql.append("   (select price from  ejd_cust_procs.get_sell_price(?, item_entity_attr.item_ea_id)) as cost, ");
            sql.append("   ejd_price_procs.get_retail_price(?, item_entity_attr.item_ea_id) as retail, ");
            sql.append("   length, width, height, weight, ");
            sql.append("   cube, retail_unit.unit uom, ejd_item.flc_id, mdc.mdc_id, mdc.nrha_id, ");
            sql.append("   item_entity_attr.retail_pack, web_item_ea.brand_name, noun, modifier ");
            sql.append("from ");
            sql.append("   item_entity_attr ");
            sql.append("join ejd_item on ejd_item.ejd_item_id = item_entity_attr.ejd_item_id ");
            sql.append("join ejd_item_warehouse on ejd_item_warehouse.ejd_item_id = item_entity_attr.ejd_item_id and ejd_item_warehouse.warehouse_id = 11 ");
            sql.append("join ejd_item_whs_upc on ejd_item_warehouse.ejd_item_id = ejd_item_whs_upc.ejd_item_id and ejd_item_whs_upc.warehouse_id = 11 and ejd_item_whs_upc.primary_upc = 1 ");
            sql.append("join item_disp on ejd_item_warehouse.disp_id = item_disp.disp_id and item_disp.disposition = 'BUY-SELL' ");
            sql.append("join web_item_ea on web_item_ea.item_ea_id = item_entity_attr.item_ea_id ");            
            sql.append("left join vendor_item_ea_cross vic on vic.vendor_id = item_entity_attr.vendor_id and vic.item_ea_id = item_entity_attr.item_ea_id ");
            sql.append("join vendor on vendor.vendor_id = item_entity_attr.vendor_id ");
            sql.append("join retail_unit on retail_unit.unit_id = item_entity_attr.ret_unit_id ");
